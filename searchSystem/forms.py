...
class SignIn(UserCacheMixin, forms.Form):
    password = forms.CharField(label=_('密码'), strip=False, widget=forms.PasswordInput)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        if settings.USE_REMEMBER_ME:
            self.fields['remember_me'] = forms.BooleanField(label=_('记住我'), required=False)
    ...
    def clean_password(self):
        ...
        if not self.user_cache.check_password(password):
            raise ValidationError(_('你输入的密码无效。'))

        return password


class SignInViaUsernameForm(SignIn):
    username = forms.CharField(label=_('用户名'),        widget=widgets.TextInput(attrs={"class": "form-control"}))
...
class SignUpForm(UserCreationForm):
    ...
    email = forms.EmailField(label=_('Email'), help_text=_('必需。请输入正确的电子邮件地址。'))
